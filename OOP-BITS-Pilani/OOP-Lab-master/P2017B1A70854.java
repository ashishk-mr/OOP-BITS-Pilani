			...
			synchronized(gameData.lock1)
			{
				...
				while(gameData.playerSuccessFlag[0]==false && gameData.playerSuccessFlag[1]==false){
					...
					while(gameData.playerChanceFlag[0]==false || gameData.playerChanceFlag[1]==false) {
						out.println("Waiting dealer player chance flag");
						gameData.lock1.wait(1000);
					}
					gameData.playerChanceFlag[0]=false;
					gameData.playerChanceFlag[1]=false;
					this.numberAnnounced=0;
				...
				if(gameData.playerSuccessFlag[0]==false && gameData.playerSuccessFlag[1]==true){ 
					// UNCOMMENT THE ONE LINE OF CODE WRITTEN BELOW AFTER SPECIFYING THE CONDITION
		...
		try{
			...
			{			
				...
				while(gameData.gameCompleteFlag == false){/////////////////////////////
					System.out.println("hsgdjsdhjd");
					out.println(totalNumbersFound);
					out.println("Ticket length: "+this.ticket.length);
					...
					while(gameData.noAnnouncedFlag==false && gameData.playerChanceFlag[0]==false && gameData.playerChanceFlag[1]==true){/////////////////
						gameData.lock1.wait(1000);
						out.println("AJAJKL");
					}
					...
					if(!gameData.gameCompleteFlag) {					
						...
						for(int i=0; i<this.ticket.length; ++i){
							...
							this.btnOnTicket[i].setBackground(Color.RED);
						...
						System.out.println("player "+id+" chance set");
